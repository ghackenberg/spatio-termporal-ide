...
public class SyntaxFeatureDurationAggregation extends DurationAggregation<Class<?>> {

	public SyntaxFeatureDurationAggregation(List<Change> changes, List<Event> events) {
		super(changes, events);
	}

	@Override
	protected DurationDescriptor<Class<?>> generateResult() {
		Map<Class<?>, List<Long>> durations = new HashMap<>();
			...
			if (event instanceof MarkerAddEvent) {
				...
				if (feature != null) {
					if (!adds.containsKey(element)) {
						...
						if (!durations.containsKey(feature)) {
							durations.put(feature, new ArrayList<>());
						}
						...
						durations.get(feature).add(duration);
