...
public class SyntaxFeatureTimelineAggregation extends TimelineAggregation<Class<?>> {

	public SyntaxFeatureTimelineAggregation(List<Change> changes, List<Event> events, Discretizer discretizer) {
				...
				if (feature != null) {
					if (!markers.containsKey(element)) {
						...
						if (!counts.containsKey(feature)) {
							counts.put(feature, new ArrayList<>());
							for (int bin = 0; bin < getDiscretizer().getBins(); bin++) {
								counts.get(feature).add(0);
							}
						}
						...
						for (int bin = startBin; bin <= endBin; bin++) {
							counts.get(feature).set(bin, counts.get(feature).get(bin) + 1);
						}
